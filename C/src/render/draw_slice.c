...
/*   Updated: 2015/01/30 18:12:50 by npineau          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_player const p)
{
	...
	start = new_pixel(new_pos(
				x, max_up((-h * (2 - p.pos.z) + W_HEIGHT * p.cam.z) / 2), 0),
			wall);
	end = new_pixel(
			new_pos(x, max_down((h * p.pos.z + W_HEIGHT * p.cam.z) / 2), 0),
			wall);
